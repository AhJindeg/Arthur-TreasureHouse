...
/**
 * 数值分割符 _
 * 数值分割符 _，在数值组之间提供分隔，使一个长数值读起来更容易。
 * 十进制的小数部分也可以使用数值分割符，二进制、十六进制里也可以使用数值分割符
 */
 ...
 * ?. 操作符的功能类似于 . 链式操作符
 * 不同之处在于，在引用为 null 或者 undefined 的情况下不会引起错误，该表达式短路返回值是 undefined
...
const aa = new Person()
console.log(aa.age) // undefined 直接访问不到
console.log(aa.getDesc()) // foo 23

/**
 * 位运算符 >> 与 >>>
 * JS 里面没有无符号左移，而且其他语言比如 JAVA 也没有无符号左移。
 */
 ...
 */
// 使用位运算符时会抛弃小数位，我们可以利用这个特性来给数字取整，比如给任意数字 & 上二进制的 32 个 1，或者 | 上 0
// 1.3 | 0// 1
// -1.9 | 0// -1
...
// 还可以用在箭头函数中避免传值泄漏，箭头函数，允许在函数体不使用括号来直接返回值。
